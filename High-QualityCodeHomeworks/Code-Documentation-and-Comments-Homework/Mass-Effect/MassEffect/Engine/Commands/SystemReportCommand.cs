namespace MassEffect.Engine.Commands
{
    ...
    public class SystemReportCommand : Command
    {
        ...
        public override void Execute(string[] commandArgs)
        {
            ...
            var intactShips =
                starships.Where(s => s.Health > 0).OrderByDescending(s => s.Health).ThenByDescending(s => s.Shields);

            StringBuilder output = new StringBuilder();
            output.AppendLine("Intact ships:");
            output.AppendLine(intactShips.Any() ? string.Join("\n", intactShips) : "N/A");

            var destroyedShips = starships.Where(s => s.Health <= 0).OrderBy(s => s.Name);
